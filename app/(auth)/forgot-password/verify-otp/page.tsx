    ...
    <div className="w-full overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <InputOTPGroup className="gap-1 sm:gap-2">
              {[...Array(6)].map((_, index) => (
                <InputOTPSlot
                  key={index}
                  index={index}
                  className={`w-[2.15rem] h-8.5 xss:w-11 xss:h-10 text-base sm:text-lg font-medium rounded-sm border-2 transition-all duration-200 ${
                    otpError
        ...
        <button
          ...
          className={`w-full mt-6.5 ${inter.variable} bg-[#39089D] hover:bg-[#39089DD9] active:bg-[#2D067E] disabled:bg-[#F6F6F6] disabled:text-[#C1C1C2] dark:bg-[#4309B6] dark:hover:bg-[#4d0ad1] dark:active:bg-[#33078c] dark:disabled:bg-[#1C1C1C] dark:disabled:text-[#5A5A5A] dark:bg-[linear-gradient(180deg,rgba(255,255,255,0.12)_0%,rgba(255,255,255,0)_100%)] shadow-xs shadow-[#0A0D120D] text-white font-medium py-3 px-6 rounded-3xl transition-all duration-200 transform outline-0 text-xs! cursor-pointer disabled:cursor-not-allowed`}
        >
          {isSubmitting ? (
